...
def debug():
    ...
    ust_balance = int(native_token[0].get("uusd").amount)
    aust_balance = dexbot.get_cw_token(AUST_CONTRACT).get("balance")
    luna_balance = int(native_token[0].get("uluna").amount)
    bluna_balance = dexbot.get_cw_token(BLUNA_CONTRACT).get("balance")
    print(
        f"{ust_balance}UST {aust_balance}aUST {luna_balance}LUNA {bluna_balance}bLUNA"
    )
    # dexbot.swap_luna_to_ust(dexbot.denom_conversion(1, is_str=True), "0.1")
